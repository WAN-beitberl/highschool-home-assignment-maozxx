...
public class viewTabl {
    public static void main(String[] args) {
        try {
            Connection connection = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/newjdbc",
                    "root", "zxczxc123");
            BufferedReader br = new BufferedReader(new FileReader("C:\\SQL2022\\highschool_sql_assignment\\highschool.csv"));
            ...
            while ((line = br.readLine()) != null) {
                ...
                PreparedStatement stmt = connection.prepareStatement("INSERT INTO newjdbc.highschool (id, first_name, last_name, email, gender, ip_address, cm_height, age, has_car, car_color, grade, grade_avg, identification_card) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE id=?");
                stmt.setInt(1, id);
            ...
            connection = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/newjdbc",
                    "root", "zxczxc123");
            br = new BufferedReader(new FileReader("C:\\SQL2022\\highschool_sql_assignment\\highschool_friendships.csv"));
            firstLine = true;
            while ((line = br.readLine()) != null) {
                if (firstLine) {
                    firstLine = false;
                ...
                PreparedStatement stmt = connection.prepareStatement("INSERT INTO newjdbc.highschool_friendships (id, friend_id, other_friend_id) VALUES (?,?,?) ON DUPLICATE KEY UPDATE id=?");
                stmt.setInt(1, id);
